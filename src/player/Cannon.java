...
public class Cannon {
	...
	public void drawCannon(Graphics g, int x, int y, double orientation) {
		...
		if (activeBertha) {
			// g.fillOval((int)bertha.getX(), (int)bertha.getY(), (int)bertha.getWidth(),
	...
	public void updateCannon(ArrayList<Obstacle> obs, ArrayList<Player> players, Player player) {
		...
		for (Bullet b : bullets) {
			...
			if (p != null && !p.getTank().isInvinsible() && p.getTeam() != owner.getOwner().getTeam()) {
				haveToRemove = b;
				p.deleteTank();
		...
		if (haveToRemove != null) {
			if (haveToRemove.isBertha()) {
				bertha.setFrame((int) (haveToRemove.getX() - bertha.getWidth() / 2),
						(int) (haveToRemove.getY() - bertha.getHeight() / 2), (int) bertha.getWidth(),
						(int) bertha.getHeight());
				activeBertha = true;
